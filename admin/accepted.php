              ...
              <div class="col-lg-12" style="padding-left:10px">
                ...
                <div class="col-lg-12">
                  ...
                  if (empty($filter) && empty($coi) && empty($at) && empty($vi)) {
                    ...
                    $result = mysqli_query($conn, "SELECT * FROM `cmp_log` where CategoryOfIssue='$coi' and cmp_log.ref_no in (select stats.ref_no from `stats` where status=4 in ($aid))");
                  } else if (!empty($at)) {
                    $result = mysqli_query($conn, "SELECT * FROM `cmp_log` where availability='$at' and cmp_log.ref_no in (select stats.ref_no from `stats` where status=4 in ($aid))");
                  } else {
                    $result = mysqli_query($conn, "SELECT * FROM `cmp_log` where visibility='$vi' and cmp_log.ref_no in (select stats.ref_no from `stats` where status=4 in ($aid))");
                  }

              ...
              <div class="col-lg-12" style="padding-left:10px">
                ...
                <div class="col-lg-12">
                  ...
                  if (empty($filter) && empty($coi) && empty($at)) {
                    $result = mysqli_query($conn, "SELECT * FROM `cmp_log` where cmp_log.ref_no in (select stats.ref_no from `stats` where status in (4))");
                  } else if (!empty($filter)) {
                    $result = mysqli_query($conn, "SELECT * FROM `cmp_log` where nameOfHostel='$filter' AND cmp_log.ref_no in (select stats.ref_no from `stats` where status in (4))");
                  } else if (!empty($coi)) {
                    $result = mysqli_query($conn, "SELECT * FROM `cmp_log` where CategoryOfIssue='$coi' and cmp_log.ref_no in (select stats.ref_no from `stats` where status in (4))");
                  } else if (!empty($at)) {
                    $result = mysqli_query($conn, "SELECT * FROM `cmp_log` where availability='$at' and cmp_log.ref_no in (select stats.ref_no from `stats` where status in (4))");
                  } else {
                    $result = mysqli_query($conn, "SELECT * FROM `cmp_log` where visibility='$vi' and cmp_log.ref_no in (select stats.ref_no from `stats` where status in (4))");
                  }
